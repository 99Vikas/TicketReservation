...
import com.railway.ticketReservation.exception.ApiRequestException;
import com.railway.ticketReservation.exception.TicketNotFoundException;
import com.railway.ticketReservation.model.*;
import com.railway.ticketReservation.service.TicketsService;
...
//                        simpleDateFormat.setTimeZone(TimeZone.getTimeZone("GMT"));
                ...
                if (tripScheduleData.getSleeperAvailableSeats() >= seatsReq.get(sleeper)) {
                    for (int i = 1; i <= seatsReq.get(sleeper); i++) {
                        if (assignedSeats.containsKey(sleeper)) {
                            ...
                            assignedSeats.put(sleeper, new ArrayList<>(List.of(train.getSleeperSeats() - tripScheduleData.getSleeperAvailableSeats() + i)));
                        }
                    }
            ...
            ticket.setUserId("vikas@gmail.com");
            // PNR = JourneyDate + class Code + first seat number
            String PNR = tripScheduleData.getTripDate().toString().replaceAll("-","")+assignedSeats.keySet().toArray()[0]+ assignedSeats.values().stream().findFirst().get().get(0);
